...
apkUtilsMethods.waitForActivityOrNot = async function waitForActivityOrNot (pkg, activity, waitForStop, waitMs = 20000) {
  ...
  const possiblePackagePatterns = possiblePackageNames.map(
    (pkgName) => new RegExp(`^${pkgName.replace(/\./g, '\\.').replace(/\*/g, '.*?').replace(/\$/g, '\\$')}$`)
  );
  ...
  const conditionFunc = async () => {
    ...
    if (appActivity && appPackage) {
      ...
      const isPackageFound = possiblePackagePatterns.some((p) => p.test(appPackage));
      const isActivityFound = _.includes(allPackages, appPackage)
        && possibleActivityPatterns.some((p) => p.test(fullyQualifiedActivity));
      if ((!waitForStop && isActivityFound && isPackageFound) || (waitForStop && !isActivityFound && !isPackageFound)) {
        return true;
      }
    }
